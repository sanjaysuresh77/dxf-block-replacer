...
def main():
    st.title("TEMPLATE CHANGE TOOL")
    st.markdown("Upload a ZIP of DXF Files and a Master Template DXF File.")

    zip_file = st.file_uploader("Upload Files for Changing Template", type=["zip"])
    template_file = st.file_uploader("Upload Master Template", type=["dxf"])    
    block_name = st.text_input("Enter the Master Template Block Name")
        ...
        with st.spinner("Processing..."):
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with open(out_zip_path, "rb") as f:
                    st.success(f"{success_count} Files Template Have Changed Successfully.")
                    st.download_button("Download Updated Template File", f, file_name="updated_dxf.zip")
